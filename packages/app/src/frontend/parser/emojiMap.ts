// Taken from Revolts front-end source code
...
/*
const toCodePoint = (rune: string) => codePoints(
  rune.indexOf(U200D) < 0 ? rune.replace(UFE0Fg, "") : rune)
...
const toCodePoint = (emojiName: string) =>
  emojiDictionary[emojiName]?.codePointAt(0).toString(16)

export const mkEmojiHref = (pack: string, emoji: string) =>
  [
    `https://static.revolt.chat/emoji/${pack}`,
    `/${toCodePoint(emoji)}.svg?rev=${REVISION}`,
  ].join('')
